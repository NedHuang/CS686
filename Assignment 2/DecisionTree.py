...
trainDataSparse=csr_matrix((np.ones(np.size(trainData,0)), (trainData[:,0]-1, trainData[:,1]-1)),\
                           shape=(max(trainData[:,0]), nWords)).todense()      
    ...
    for i in range(0,nWords):
        if (E[:,i].sum()!=0) and (i not in used_feature):
            ...
            if p_0[0]==0:
                ...
                IE1_1=-(p_0[0]*math.log2(p_0[0]))

            if p_0[1]==0:
                IE1_2=0
            else:
                IE1_2=-(p_0[1]*math.log2(p_0[1]))

            IE1 = IE1_1+IE1_2
            if p_1[0]==0:
                IE2_1=0
            else:
                IE2_1=-(p_1[0]*math.log2(p_1[0]))

            if p_1[1]==0:
                IE2_2=0
            else:
                IE2_2=-(p_1[1]*math.log2(p_1[1]))
...
E[np.where(E[:,DT[0][0]]==0)[0],:]

(trainDataSparse[:,DT[0][0]]==0).sum()