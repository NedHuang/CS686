...
############################
    ...
    train_data = np.loadtxt(fname="trainData.txt",dtype=int)
    test_data = np.loadtxt(fname="testData.txt",dtype=int)
    train_label = np.loadtxt(fname="trainLabel.txt",dtype=int)
    test_label = np.loadtxt(fname="testLabel.txt",dtype=int)
    train_data[:,0] -= 1
    train_data[:,1] -= 1
    ...
    # data /label -> matrix
    # csr_matrix((data, (row, col)), shape=(n, m))
    train_data = csr_matrix(
        (np.ones(np.size(train_data,0)), (train_data[:,0], train_data[:,1])),
        shape=(max(train_data[:,0])+1, wordIdMax)
    ).toarray()

    test_data = csr_matrix(
        (np.ones(np.size(test_data,0)),(test_data[:,0], test_data[:,1])),
        shape=(max(test_data[:,0])+1, wordIdMax)
    ).toarray()
    print(train_data.shape)
    print(train_label.shape)
    print(test_data.shape)
